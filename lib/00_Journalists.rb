def journalist
	...
	puts " "
	puts "Tri de la liste de handle par ordre alphabétique:"
	#listsort1 = list.sort_by {|value| x.length}
	listsort1 = list.sort(&:casecmp)
	#puts listsort1.sort
	listsort1.each do |value|
	 	print "#{value}, "
	end

	puts " "
	puts "Quels sont les 50 handles les plus courts de ce array ?"
	listsort2 = list.sort_by {|x| x.length}
	listsort3 = listsort2[0 .. 49]
	#print "#{key},"
	listsort3.each do |value|
	 	print "#{value}, "
	end
	puts " "
	...
	test_var = 0
	while test_var == 0
		countepenser = countepenser + 1 
		str = list[countepenser]
		if str == ("@epenser")
	 		test_var = 1
		end
	end
